...
class FR3BoundingBoxSim(Env):
    ...
    def __init__(self, render_mode: Optional[str] = None, record_path=None):
        ...
        self.active_joint_ids = [0, 1, 2, 4, 6, 9, 11, 17, 18]
        ...
        self.FR3_LINK3_BOUNDING_BOX_FRAME_ID = self.robot.model.getFrameId(
            "fr3_link3_bounding_box"
        )
        self.FR3_LINK4_FRAME_ID = self.robot.model.getFrameId("fr3_link4")
        self.FR3_LINK4_BOUNDING_BOX_FRAME_ID = self.robot.model.getFrameId(
            "fr3_link4_bounding_box"
        )
        self.FR3_LINK5_FRAME_ID = self.robot.model.getFrameId("fr3_link5")
        self.FR3_LINK5_1_BOUNDING_BOX_FRAME_ID = self.robot.model.getFrameId(
            "fr3_link5_1_bounding_box"
        )
        self.FR3_LINK5_2_BOUNDING_BOX_FRAME_ID = self.robot.model.getFrameId(
            "fr3_link5_2_bounding_box"
        )
        self.FR3_LINK6_FRAME_ID = self.robot.model.getFrameId("fr3_link6")
        self.FR3_LINK6_BOUNDING_BOX_FRAME_ID = self.robot.model.getFrameId(
            "fr3_link6_bounding_box"
        )
        self.FR3_LINK7_FRAME_ID = self.robot.model.getFrameId("fr3_link7")
        self.FR3_LINK7_BOUNDING_BOX_FRAME_ID = self.robot.model.getFrameId(
            "fr3_link7_bounding_box"
        )
        self.FR3_HAND_FRAME_ID = self.robot.model.getFrameId("fr3_hand")
        self.FR3_HAND_BOUNDING_BOX_FRAME_ID = self.robot.model.getFrameId(
            "fr3_hand_bounding_box"
        )
    ...
    def get_info(self, q, dq):
        ...
        jacobian_link3 = self.robot.getFrameJacobian(
            self.FR3_LINK3_BOUNDING_BOX_FRAME_ID, self.jacobian_frame
        )
        jacobian_link4 = self.robot.getFrameJacobian(
            self.FR3_LINK4_BOUNDING_BOX_FRAME_ID, self.jacobian_frame
        )
        jacobian_link5_1 = self.robot.getFrameJacobian(
            self.FR3_LINK5_1_BOUNDING_BOX_FRAME_ID, self.jacobian_frame
        )
        jacobian_link5_2 = self.robot.getFrameJacobian(
            self.FR3_LINK5_2_BOUNDING_BOX_FRAME_ID, self.jacobian_frame
        )
        jacobian_link6 = self.robot.getFrameJacobian(
            self.FR3_LINK6_BOUNDING_BOX_FRAME_ID, self.jacobian_frame
        )
        jacobian_link7 = self.robot.getFrameJacobian(
            self.FR3_LINK7_BOUNDING_BOX_FRAME_ID, self.jacobian_frame
        )
        ...
        p_link3, R_link3, q_LINK3 = self.compute_crude_location(
            np.eye(3), np.zeros((3, 1)), self.FR3_LINK3_BOUNDING_BOX_FRAME_ID
        )

        p_link4, R_link4, q_LINK4 = self.compute_crude_location(
            np.eye(3), np.zeros((3, 1)), self.FR3_LINK4_BOUNDING_BOX_FRAME_ID
        )

        p_link5_1, R_link5_1, q_LINK5_1 = self.compute_crude_location(
            np.eye(3), np.zeros((3, 1)), self.FR3_LINK5_1_BOUNDING_BOX_FRAME_ID
        )

        p_link5_2, R_link5_2, q_LINK5_2 = self.compute_crude_location(
            np.eye(3), np.zeros((3, 1)), self.FR3_LINK5_2_BOUNDING_BOX_FRAME_ID
        )

        p_link6, R_link6, q_LINK6 = self.compute_crude_location(
            np.eye(3), np.zeros((3, 1)), self.FR3_LINK6_BOUNDING_BOX_FRAME_ID
        )

        p_link7, R_link7, q_LINK7 = self.compute_crude_location(
            np.eye(3), np.zeros((3, 1)), self.FR3_LINK7_BOUNDING_BOX_FRAME_ID
        )

        p_hand, R_hand, q_HAND = self.compute_crude_location(
            np.eye(3), np.zeros((3, 1)), self.FR3_HAND_BOUNDING_BOX_FRAME_ID
        )
